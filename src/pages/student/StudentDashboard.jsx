...
const StudentDashboard = () => {
  ...
  // Determinar mensaje según la tasa de aprobación
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Sección de horas y tasa de aprobación */}
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-3">
          Horas de Servicio
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className="text-4xl font-bold text-gray-900">
              {stats.total_hours_submitted || 0}
              <span className="text-lg text-gray-500 ml-1">h</span>
            </p>
            <p className="text-xs text-gray-500 mt-2">
              Total de horas enviadas en reportes
            </p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className={`text-xs font-medium mt-2 ${approvalMessage.color}`}>
              {approvalMessage.emoji} {approvalMessage.text}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-gray-500">
              {stats.total_hours_approved || 0}h de {stats.total_hours_submitted || 0}h aprobadas
            </span>
          </div>
          ...
          <div className="relative">
            <div className="w-full bg-gray-200 rounded-full h-3 overflow-hidden">
              <div
                ...
                style={{ 
                  width: `${stats.total_hours_submitted > 0 
                    ? Math.min(100, ((stats.total_hours_approved || 0) / stats.total_hours_submitted) * 100) 
                    : 0}%` 
                }}
              />
            </div>
          ...
          <div className="flex justify-between mt-3 text-xs text-gray-500">
            ...
            <span className="font-medium text-blue-600">
              {stats.total_hours_approved || 0}h aprobadas
            </span>
            <span>{stats.total_hours_submitted || 0}h</span>
          </div>
        </div>
      )}

      {/* Mensaje motivacional si no hay reportes */}
      {stats.total === 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 text-center">
          <FileText className="w-12 h-12 text-blue-400 mx-auto mb-3" />
          <h3 className="font-semibold text-blue-900 mb-1">
            ¡Comienza tu viaje de servicio!
          </h3>
          <p className="text-sm text-blue-700">
            Aún no has enviado ningún reporte. Ve a "Nuevo Reporte" para registrar tus primeras horas de servicio.
          </p>
        </div>
      )}
    </div>
